...
class HomeViewModel(private val repository: IRecipesRepository) : ViewModel() {
    ...
    fun loadRecipes() {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                when (val result = repository.getAllRecipes()) {
                    is Result.Error -> _isError.postValue(Event(result.message))
                    is Result.Success -> {
                        _recipes.postValue(result.data!!)
                        _isSuccess.postValue(Event(""))
                        allRecipes = result.data.orEmpty()
                    }
                }
            } catch (ex: Throwable) {
            ...
            try {
                when (val result = repository.removeFromFavourite(recipeId)) {
                    ...
                    is Result.Success -> {
                        ...
                        val mov = _recipes.value!!
                        mov.find { it.id == recipeId }!!.isFavourite = false
                        _recipes.postValue(mov)
                    }
                }
            } catch (ex: Throwable) {
    ...
    fun filterRecipesByCategory(category: String?) {
        ...
        val filteredElements = if (category == null) {
            allRecipes
        } else {
            allRecipes.filter { recipe ->
        ...
        _recipes.value = filteredElements
    }
}